...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-4 top-4 border-2 border-foreground bg-card p-1 shadow-brutal-sm brutal-press opacity-80 hover:opacity-100 hover:bg-destructive hover:text-destructive-foreground focus:outline-none disabled:pointer-events-none transition-colors">
        <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn(
      "text-xl font-heading font-bold uppercase tracking-wide",
      className
    )}
    {...props}
  />
